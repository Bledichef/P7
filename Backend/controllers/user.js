...
exports.signUp = (req, res) => {
  ...
  if (regexEmail.test(email)) {
    ...
    return res
      .status(404)
      .json({ message: "Le format de l'adresse mail n'est pas correct" });
  }
};
      ...
      model.User.findOne({
        ...
        .then((user) => {
          ...
          if (userRequest.isadmin === true || userRequest.id === user.id) {
            ...
            return res
              .status(404)
              .json({ error: "Vous n'avez pas le droit de faire ça" });
          }
        })
        .catch((error) => res.status(400).json({ error }));
    })
  ...
  model.User.findOne({
    ...
    .then((userRequest) => {
      ...
      if (userRequest.isadmin === true) {
        ...
        return res
          .status(404)
          .json({ message: "Vous n'avez pas l'autorisation de faire ça" });
      }
    })
    .catch((error) => res.status(400).json({ error }));
};
          ...
          if (userRequest.isadmin === true) {
            if (!user) {
              ...
              if (regexEmail.test(email)) {
                ...
                return res
                  .status(404)
                  .json({ message: "Le format de la requête est invalide" });
              }
            }
          } else {
            return res
