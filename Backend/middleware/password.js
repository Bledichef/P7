...
passwordSchema
  ...
  .is()
  .max(100) // Doit contenir max 100 caractères
  .has()
...
module.exports = (req, res, next) => {
  ...
  if (!passwordSchema.validate(req.body.password)) {
    return res.status(400).json({
      message:
        "Le mot de passe n'est pas conforme. Réessayez avec au moins 6 caractères, 1 majuscule, 2 chiffres et 1 caractère spécial",
    });
  } else {
    next();
  }
